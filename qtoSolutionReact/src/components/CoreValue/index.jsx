    ...
    <div className="py-12 bg-gray-100">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-extrabold text-gray-800 mb-4">Our Core Values</h1>
        </div>
          ...
          {values.map((value, index) => (
            <div key={index} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 ease-in-out">
              <div className="flex flex-col items-center text-center">
                {value.icon}
                <h2 className="text-xl font-bold text-gray-800 mt-2">{value.title}</h2>
                <p className="text-justify text-gray-600 mt-2">{value.description}</p>
              </div>
            </div>
          ))}
        </div>
